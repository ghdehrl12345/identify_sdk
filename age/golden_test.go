...
func TestGoldenProofAge(t *testing.T) {
	...
	if g.VerifyingID != AgeVerifyingKeyID() {
		t.Fatalf("vk id mismatch: %s != %s", g.VerifyingID, AgeVerifyingKeyID())
	}
	...
	ok, err := verifier.VerifyAge(proofBytes)
	if err != nil || !ok {
